...
class _DeviceTrustViewState extends State<_DeviceTrustView> {
  ...
  String get _scope => 'device_trust:${widget.phone}';

  @override
  void initState() {
    super.initState();
    // Resume an active cooldown (don't waste an SMS on a quick reopen);
    // otherwise send the first code.
  ...
  Future<void> _send() async {
    final secs = await context.read<DeviceTrustCubit>().sendCode();
    if (!mounted) return;
